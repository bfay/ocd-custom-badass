<?php
function rockin_setup() {
	// Add language supports. Please note that rockin Framework does not include language files.
	load_theme_textdomain('rockin', get_template_directory() . '/lang');
	...
	register_nav_menus(array(
		'primary' => __('Primary Navigation', 'rockin'),
		'utility' => __('Utility Navigation', 'rockin')
	));	
}
add_action('after_setup_theme', 'rockin_setup');
...
function rockin_css() {
  if ( !is_admin() ) {
     ...
     wp_register_style( 'foundation',get_template_directory_uri() . '/css/rockin.css', false );
...
add_action( 'init', 'rockin_css' );

function rockin_ie_css () {
    echo '<!--[if lt IE 9]>';
    echo '<link rel="stylesheet" href="'. get_template_directory_uri().'/css/ie.css">';
    echo '<![endif]-->';
}
add_action( 'wp_head', 'rockin_ie_css' );

// Enqueue js files
function rockin_scripts() {
...
add_action( 'init', 'rockin_scripts' );
...
function rockin_entry_meta() {
	echo '<time class="updated" datetime="'. get_the_time('c') .'" pubdate>'. sprintf(__('Posted on %s at %s.', 'rockin'), get_the_time('l, F jS, Y'), get_the_time()) .'</time>';
	echo '<p class="byline author vcard">'. __('Written by', 'rockin') .' <a href="'. get_author_posts_url(get_the_author_meta('ID')) .'" rel="author" class="fn">'. get_the_author() .'</a></p>';
}
...
function rockin_pagination() {
	...
	if ( $paginate_links ) {
		echo '<div class="rockin-pagination">';
		echo $paginate_links;
 ...
 */
function rockin_menu_fallback() {
	echo '<div class="alert-box secondary">';
	// Translators 1: Link to Menus, 2: Link to Customize
  	printf( __( 'Please assign a menu to the primary menu location under %1$s or %2$s the design.', 'rockin' ),
  		sprintf(  __( '<a href="%s">Menus</a>', 'rockin' ),
  			get_admin_url( get_current_blog_id(), 'nav-menus.php' )
  		),
  		sprintf(  __( '<a href="%s">Customize</a>', 'rockin' ),
...
function rockin_active_nav_class( $classes, $item ) {
    if ( $item->current == 1 || $item->current_item_ancestor == true ) {
        $classes[] = 'active';
    }
    return $classes;
}
add_filter( 'nav_menu_css_class', 'rockin_active_nav_class', 10, 2 );

/**
 * Use the active class of ZURB Foundation on wp_list_pages output.
 * From required+ Foundation http://themes.required.ch
 */
function rockin_active_list_pages_class( $input ) {
...
add_filter( 'wp_list_pages', 'rockin_active_list_pages_class', 10, 2 );
 ...
 * From required+ Foundation http://themes.required.ch
 */
class rockin_walker extends Walker_Nav_Menu {
